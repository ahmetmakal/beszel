...
export const alertInfo: Record<string, AlertInfo> = {
	...
	IOWait: {
		...
		unit: "%",
		icon: HourglassIcon,
		desc: () => t`Triggers when CPU I/O wait exceeds a threshold (hypervisor disk bottleneck signal)`,
		start: 10,
		step: 1,
	},
	DiskAwait: {
		...
		unit: " ms",
		icon: HardDriveIcon,
		desc: () => t`Triggers when disk read or write await exceeds a threshold`,
		start: 20,
		step: 5,
		max: 500,
	},
	VMBlocked: {
		...
		unit: "",
		icon: ServerIcon,
		desc: () => t`Triggers when any libvirt/KVM VM enters blocked state`,
		singleDesc: () => t`Any VM blocked`,
	},
	Vulnerability: {
